...
## 🔴 JSON 저장/불러오기 호환성 (최우선 원칙!)

### 절대 규칙
```
⚠️ 저장된 JSON 파일을 불러왔을 때, 저장 당시의 화면을 100% 동일하게 재현해야 한다!
```

### 코드 변경 시 체크리스트
```
□ 새 필드 추가 시 → defaultTourData에 기본값 필수 추가
□ 필드 삭제 시 → 기존 JSON에 해당 필드가 있어도 에러 없이 무시
□ 필드 타입 변경 시 → 마이그레이션 로직 또는 타입 변환 처리
□ 필드명 변경 시 → 기존 필드명도 함께 처리 (하위 호환성)
□ 컴포넌트 구조 변경 시 → 기존 데이터로도 렌더링 가능하도록
```

### 데이터 병합 패턴 (필수 적용)
```typescript
// ✅ 올바른 패턴: 기본값과 병합
const loadedData = { ...defaultTourData, ...savedData };

// ❌ 잘못된 패턴: 저장된 데이터만 사용
const loadedData = savedData; // 새 필드 누락됨!
```

### 테스트 시나리오 (변경 전 확인)
```
1. 현재 버전에서 사이트 저장 → JSON 파일 생성
2. 코드 변경 후
3. 이전에 저장한 JSON 파일 불러오기
4. 저장 당시와 동일한 화면이 표시되는지 확인
```

### 영향받는 파일들
- `src/types/tour-data.ts` - TourData 인터페이스, defaultTourData
- `src/hooks/useTourData.ts` - 데이터 로드/저장 로직
- `src/App.tsx` - loadSettings, saveSettings 함수
- 모든 페이지 컴포넌트 - 데이터 렌더링 로직
